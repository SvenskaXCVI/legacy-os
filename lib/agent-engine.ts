...
export async function executeAgentTask(taskId: string, workspaceId: string, db: Db = getDb()) {
  ...
  if (specialist) {
    await db.batch([
      ...
      db.insert(usageEvents).values({ id: makeId("usage"), workspaceId, runId, provider: specialist.provider, model: specialist.model, inputTokens: specialist.inputTokens, outputTokens: specialist.outputTokens, occurredAt: completedAt.toISOString() }),
    ]);

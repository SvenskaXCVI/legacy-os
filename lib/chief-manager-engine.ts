...
export async function runChiefManager(input: { workspaceId: string; requestedBy: string; objective?: string; mode?: "operating_brief" | "command"; projectId?: string | null; clientId?: string | null; requestedTool?: string | null; actionPayload?: Record<string, unknown>; idempotencyKey: string }, db: Db = getDb()) {
  ...
  await db.batch([
    ...
    db.insert(usageEvents).values({ id: makeId("usage"), workspaceId: input.workspaceId, runId: aiRunId, provider, model, inputTokens, outputTokens, cachedInputTokens, reasoningTokens, pricingVersion: provider === "Legacy OS" ? "local-rules" : "provider-invoice-required", occurredAt: completedAt }),
